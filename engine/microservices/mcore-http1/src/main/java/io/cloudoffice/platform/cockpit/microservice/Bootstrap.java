...
import io.cloudoffice.platform.cockpit.router.RestWANPort;
...
public abstract class Bootstrap {
    ...
    public Bootstrap(Class... microservice) {
        this.context=new Context();
        initContext();
        Bootstrap.ROUTER = new Router(context().get(Keys.HOST), Integer.parseInt(context().get(Keys.PORT)), BASEDIR, context(), microservice);
    }
    ...
    public static void start(Bootstrap boot) {
        try {
            Bootstrap.ROUTER.bindWANPort(RestWANPort.class).activate();
        } catch (Exception ex) {
