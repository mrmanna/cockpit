...
/**
 * <p>
 * <em>This API is very immature.</em> The Http2Connection-based API is
 * currently preferred over this API. This API is targeted to eventually replace
 * or reduce the need for the {@link Http2ConnectionHandler} API.
 *
 * <p>
 * A HTTP/2 handler that maps HTTP/2 frames to {@link Http2Frame} objects and
 * vice versa. For every incoming HTTP/2 frame, a {@link Http2Frame} object is
 * created and propagated via {@link #channelRead}. Outbound {@link Http2Frame}
 * objects received via {@link #write} are converted to the HTTP/2 wire format.
 * HTTP/2 frames specific to a stream implement the {@link Http2StreamFrame}
 * interface. The {@link Http2FrameCodec} is instantiated using the
 * {@link Http2FrameCodecBuilder}. It's recommended for channel handlers to
 * inherit from the {@link Http2ChannelDuplexHandler}, as it provides additional
 * functionality like iterating over all active streams or creating outbound
 * streams.
 *
 * <h3>Stream Lifecycle</h3>
 *
 * The frame codec delivers and writes frames for active streams. An active
 * stream is closed when either side sends a {@code RST_STREAM} frame or both
 * sides send a frame with the {@code END_STREAM} flag set. Each
 * {@link Http2StreamFrame} has a {@link Http2FrameStream} object attached that
 * uniquely identifies a particular stream.
 *
 * <p>
 * {@link Http2StreamFrame}s read from the channel always a
 * {@link Http2FrameStream} object set, while when writing a
 * {@link Http2StreamFrame} the application code needs to set a
 * {@link Http2FrameStream} object using
 * {@link Http2StreamFrame#stream(Http2FrameStream)}.
 *
 * <h3>Flow control</h3>
 *
 * The frame codec automatically increments stream and connection flow control
 * windows.
 *
 * <p>
 * Incoming flow controlled frames need to be consumed by writing a
 * {@link Http2WindowUpdateFrame} with the consumed number of bytes and the
 * corresponding stream identifier set to the frame codec.
 *
 * <p>
 * The local stream-level flow control window can be changed by writing a
 * {@link Http2SettingsFrame} with the {@link Http2Settings#initialWindowSize()}
 * set to the targeted value.
 *
 * <p>
 * The connection-level flow control window can be changed by writing a
 * {@link Http2WindowUpdateFrame} with the desired window size
 * <em>increment</em> in bytes and the stream identifier set to {@code 0}. By
 * default the initial connection-level flow control window is the same as
 * initial stream-level flow control window.
 *
 * <h3>New inbound Streams</h3>
 *
 * The first frame of a HTTP/2 stream must be a {@link Http2HeadersFrame}, which
 * will have a {@link Http2FrameStream} object attached.
 *
 * <h3>New outbound Streams</h3>
 *
 * A outbound HTTP/2 stream can be created by first instantiating a new
 * {@link Http2FrameStream} object via
 * {@link Http2ChannelDuplexHandler#newStream()}, and then writing a
 * {@link Http2HeadersFrame} object with the stream attached.
 *
 * <pre>
 ...
 *
 * <p>
 * If a new stream cannot be created due to stream id exhaustion of the
 * endpoint, the {@link ChannelPromise} of the HEADERS frame will fail with a
 * {@link Http2NoMoreStreamIdsException}.
 *
 * <p>
 * The HTTP/2 standard allows for an endpoint to limit the maximum number of
 * concurrently active streams via the {@code SETTINGS_MAX_CONCURRENT_STREAMS}
 * setting. When this limit is reached, no new streams can be created. However,
 * the {@link Http2FrameCodec} can be build with
 * {@link Http2FrameCodecBuilder#encoderEnforceMaxConcurrentStreams(boolean)}
 * enabled, in which case a new stream and its associated frames will be
 * buffered until either the limit is increased or an active stream is closed.
 * It's, however, possible that a buffered stream will never become active. That
 * is, the channel might get closed or a GO_AWAY frame might be received. In the
 * first case, all writes of buffered streams will fail with a
 * {@link Http2ChannelClosedException}. In the second case, all writes of
 * buffered streams with an identifier less than the last stream identifier of
 * the GO_AWAY frame will fail with a {@link Http2GoAwayException}.
 *
 * <h3>Error Handling</h3>
 *
 * Exceptions and errors are propagated via
 * {@link ChannelInboundHandler#exceptionCaught}. Exceptions that apply to a
 * specific HTTP/2 stream are wrapped in a {@link Http2FrameStreamException} and
 * have the corresponding {@link Http2FrameStream} object attached.
 *
 * <h3>Reference Counting</h3>
 *
 * Some {@link Http2StreamFrame}s implement the {@link ReferenceCounted}
 * interface, as they carry reference counted objects (e.g. {@link ByteBuf}s).
 * The frame codec will call {@link ReferenceCounted#retain()} before
 * propagating a reference counted object through the pipeline, and thus an
 * application handler needs to release such an object after having consumed it.
 * For more information on reference counting take a look at
 * http://netty.io/wiki/reference-counted-objects.html
 *
 * <h3>HTTP Upgrade</h3>
 *
 * Server-side HTTP to HTTP/2 upgrade is supported in conjunction with
 * {@link Http2ServerUpgradeCodec}; the necessary HTTP-to-HTTP/2 conversion is
 * performed automatically.
 */
@UnstableApi
public class Http2FrameCodecX extends Http2ConnectionHandler {
    ...
    Http2FrameCodecX(Http2ConnectionEncoder encoder, Http2ConnectionDecoder decoder, Http2Settings initialSettings) {
    ...
    /**
     * Processes all {@link Http2Frame}s. {@link Http2StreamFrame}s may only
     * originate in child streams.
     */
    @Override
    ...
    private void writeHeadersFrame(
        ...
        if (isStreamIdValid(headersFrame.stream().id())) {
            if (headersFrame.isPushPromise()) {
                encoder().writePushPromise(ctx, encoder().connection().remote().lastStreamCreated(), headersFrame.stream().id(), headersFrame.headers(), headersFrame.padding(), promise);
            } else {
            ...
            if (headersFrame.isPushPromise()) {
                int remoteStreamId = encoder().connection().remote().lastStreamCreated();
                encoder().writePushPromise(ctx, remoteStreamId, streamId, headersFrame.headers(), headersFrame.padding(), writePromise);
            } else {
     ...
     * {@link Http2FrameStream} object attached are simply logged and replied to
     * by sending a RST_STREAM frame.
     */
    @Override
    ...
    private final class FrameListener implements Http2FrameListener {
        ...
        public void onHeadersRead(ChannelHandlerContext ctx, int streamId,
                Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding, boolean endStream) {
