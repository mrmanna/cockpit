...
public class AuthMicroservice extends Microservice {
    ...
    public static final String signinEndpoint = "/api/jsonws/VirtualItem-portlet.virtualpersonitem/loginFromMD/username/{0}/password/{1}/company/{2}/time-zone/ASIA";
    ...
    public void signout(final @Context ContainerRequest request, final String content) {
        ...
        Captain
                ...
                .begin((duty) -> {
                    ...
                    ResponseBuilder res = Response.ok(result)
                            .cookie(signinCookie)
                            .header("Cache-Control", "no-cache");
                    request.getCookies().entrySet().stream().forEach(t -> {
                        if (!t.getKey().equalsIgnoreCase(Microservice.TOKEN_KEY)) {
                            res.cookie(new NewCookie(t.getValue().getName(), t.getValue().getValue()));
                        }
                    });
                    put(duty.captain().assignment().channel(), "result", res.build());
                    duty.captain().deliverAndCloseAssignment();
                }).catchError((error) -> {
    ...
    public void signin(final @Context ContainerRequest httpRequest, String content) {
        ...
        if (!validSession(httpRequest)) {
            ...
            if (validSigninRequest(login)) {
                PathResolver resolver = new PathResolver(httpRequest, authentic(httpRequest));
                final String username = login.getString("username");
                String path = MessageFormat.format(signinEndpoint, login.getString("username"), login.getString("password"), login.getString("company"));
                ...
                Bootstrap.ROUTER.hub().getFirst("/api").listener().listen(ask(path, httpRequest), new AuthValidator()).addListener((ChannelFuture f) -> {
                    if (f.isSuccess()) {
                        ...
                        if (null != layout) {
                            ...
                            obj.put("layout", resolver.homePath().getString("layout")); 
                        }
                        if (null != cookiepath) {
                            obj.put("path", cookiepath.getValue());
                        }else{
                             obj.put("path", resolver.homePath().getString("path")); 
                        }
