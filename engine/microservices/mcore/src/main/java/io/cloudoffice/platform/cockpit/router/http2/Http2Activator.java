...
public class Http2Activator extends DefaultRouterActivator {
    ...
    private static final UpgradeCodecFactory upgradeCodecFactory = new UpgradeCodecFactory() {
        ...
        public UpgradeCodec newUpgradeCodec(CharSequence protocol) {
            if (AsciiString.contentEquals(Http2CodecUtil.HTTP_UPGRADE_PROTOCOL_NAME, protocol)) {
                return null;//new Http2ServerUpgradeCodec(
    ...
    public Http2Activator(Router router, WANPort port) {
        super(router, port);
    }
    ...
    protected void initChannel(Channel ch) throws Exception {
        ...
        if (router().secure()) {
            p.addLast("ssl",new SecurityLayer(router().sslMap()));
            p.addLast(new Http2Nagotiator((WANPort) WANPort()));
        }
